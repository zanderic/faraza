		...
		<div class="col-md-offset-3 col-md-6">
			<div class="panel panel-success">
				<div class="panel-heading">
					<h3 class="panel-title">Elimina recensioni</h3>
				</div>
...
<?php
            $ID = $_SESSION["ID"];
            try {
                $sql = "SELECT * FROM faraza.recensione , faraza.impiantoSportivo  WHERE  recensione.IDImpiantoSportivo = impiantoSportivo.ID AND IDUtente ='$ID' ORDER BY data";
...
?>
					...
					<form action='eliminarecensione.php' method='POST'>
						<label for="num">Numero recensione</label>
						<select class="form-control" id='num' name='id_campo'>
<?php
...
?>
                	...
                	<button type='submit' class="btn btn-success btn-block">Elimina</button>
					</form>
				</div>
			</div>
		</div>
